...
| Widgets de uma fonte/sub-fonte com campo CUSTOM de período seguem lentos | Filtro de período em `custom_fields->>key` sem índice de expressão = seq scan da fonte inteira (a 0103 só cobre as 2 chaves de Data Reunião) | Rode `supabase/apply/indices-periodo-custom.sql` no SQL Editor (idempotente — cria índice parcial p/ cada `default_period_field` `custom:` configurado). Repita sempre que apontar uma fonte/sub-fonte nova p/ campo custom |
