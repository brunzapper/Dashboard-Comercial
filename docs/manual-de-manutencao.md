...
|---|---|---|
| Dashboard lento/widgets com erro após Backfill ou Reconciliar em massa | O sync reescreveu todas as linhas de `records` → tuplas mortas + estatísticas defasadas; logs do Supabase mostram `57014 statement timeout` e RPC 500; a carga simultânea dos widgets realimenta o problema | Desde a 0102 o runner dispara `maintenance_analyze()` sozinho ao concluir job com >= 2.000 linhas escritas (best-effort — procure `[sync] maintenance_analyze falhou` nos logs se suspeitar que não rodou). Persistindo o sintoma, rode a **Parte A** de `supabase/apply/diagnostico-perf.sql` (`ANALYZE`, roda no SQL editor). `VACUUM` (Parte B) é secundário e NÃO roda no SQL editor (erro 25001): deixe ao autovacuum ou force via `psql` na conexão direta. Depois confira `[dashboard:timing]` nos logs da Vercel para achar widget dominante remanescente |
| Widget mostra dado de ~1 min atrás logo após editar registro/sync | Cache TTL de `run_widget_query` entre requisições (`lib/widgets/rpc-cache.ts`, default 45s) — comportamento esperado | Aguarde o TTL ou recarregue depois. Para depurar sem cache, defina `WIDGET_RPC_CACHE_TTL_MS=0` na Vercel (desliga) |
| Gráficos/KPIs ficam em "Atualizando…" e nunca carregam (estrutura abre, dados não) | Deferimento automático dos widgets de engine (26/07/2026): a page entrega a estrutura e o `DashboardClient` busca os dados via `runDeferredWidgets`; se essa action falha (sessão/erro de rede), os cards ficam pendentes | Veja o console do navegador e os logs da Vercel (`[deferred] widget … falhou`). Para isolar, defina `DEFER_ENGINE_WIDGETS=0` na Vercel — a page volta a computar tudo no RSC (rollback sem deploy) |
...
| Widgets de uma base/sub-base com campo CUSTOM de período seguem lentos | Filtro de período em `custom_fields->>key` sem índice de expressão = seq scan da fonte inteira (a 0103 só cobre as 2 chaves de Data Reunião) | Rode `supabase/apply/indices-periodo-custom.sql` no SQL Editor (idempotente — cria índice parcial p/ cada `default_period_field` `custom:` configurado, de base 0110 e de sub 0082). Repita sempre que apontar uma base/sub-base nova p/ campo custom (config das duas em Registros → Bases) |
...
| Registros de planilha/CSV aparecem 1 dia ANTES (ex.: venda do dia 1 caindo no mês anterior — "15 vs 17 vendas do site") | Valor naive (`yyyy-MM-dd` ou hora de parede BRT) gravado em coluna core `timestamptz` SEM âncora de fuso — o Postgres assume UTC e o dia de Brasília recua (00:00 vira 21h da véspera) | Write side corrigido em 26/07/2026 (`anchorNaiveToBrasilia` no adapter de Sheets e no `ingestRows`); para o legado, rode `supabase/apply/backfill-naive-tz.sql` (bloco do Meetime roda UMA vez — ver comentários do arquivo). Snapshots congelados antes do backfill preservam o dia antigo: recapture/atualize |
| `audit_log` inchado com `source_created_at`/`source_modified_at` a cada sync (mesmo instante, formato diferente) | Reconcile comparava colunas core `timestamptz` por STRING (`valuesDiffer`): PostgREST devolve `+00:00`, mapper emite `-03:00` — toda linha "mudava" a cada sync | Corrigido em 26/07/2026 (`timestampValuesDiffer` em `lib/sync/shared.ts`, usado no sync Bitrix e no import CSV); entradas antigas do audit são inofensivas (mesmo instante) e podem ser purgadas via SQL se o volume incomodar |
