-- 0085_widget_rpc_brasilia_day.sql
-- Versão: 1.0 | Data: 21/07/2026
-- Dia de BRASÍLIA no read side dos RPCs de widgets. A sessão do banco roda em
-- UTC; até aqui os literais de período chegavam SEM offset ('2026-07-01' /
-- '2026-07-15T23:59:59') e, comparados a colunas timestamptz do núcleo
-- (source_created_at etc.), eram castados como UTC — todo limite de dia
-- deslocava 3h (registros 21:00–23:59 BRT do dia anterior ao range entravam;
-- as últimas 3h do último dia sumiam). O bucketing (date_trunc) e a
-- serialização ::text dos unificados tinham o mesmo desvio, divergindo do
-- cliente (100% prefix-based no dia de Brasília). Campos custom (texto ISO já
-- normalizado p/ -03:00 nas 0079/0080) sempre compararam certo — e seguem
-- BYTE-IDÊNTICOS aqui.
--
-- Mudanças (espelhadas em run_widget_query_snapshot NESTA migração — regra do
-- projeto; helpers compartilhados não têm twin _snap por emitirem refs de
-- coluna não qualificadas):
--   1) Novo helper _widget_local_ts com 2 OVERLOADS: timestamptz → wall time
--      de America/Sao_Paulo; text → prefixo de 10 chars (::timestamp seguro,
--      inválido → null). A resolução de overload mantém o SQL gerado uniforme
--      — inclusive nas subconsultas de match:, tipadas pela coluna do
--      parceiro. NUNCA aplicar "at time zone" em texto: valor naive (CSV)
--      '…T01:00:00' recuaria um dia; o prefixo casa byte a byte com o
--      parseYmd do cliente.
--   2) _widget_col_date_expr passa a emitir _widget_local_ts(...) nos dois
--      ramos (núcleo e custom) — retorno timestamp homogêneo; o coalesce de
--      _widget_unified_date_expr segue sem mudança de corpo. _widget_safe_ts
--      fica no lugar (legado, sem chamadores nos RPCs).
--   3) _widget_unified_expr: membro NÃO numérico que seja coluna de DATA do
--      núcleo serializa como (_widget_local_ts(col))::text — prefixo
--      YYYY-MM-DD no dia de Brasília (antes: '… +00' da sessão UTC). Membros
--      custom/text/bool intocados.
--   4) run_widget_query/_snapshot: transforms de data do núcleo, de match: e
--      do @rate_date operam sobre _widget_local_ts(...); o ramo unified: já
--      recebe timestamp do helper (NÃO embrulhar — timestamp implícito p/
--      timestamptz reinterpretaria o wall time como UTC). Formatos de chave
--      do @bucket intocados (to_char/extract sobre timestamp) — invariante do
--      canonicalBucketKey preservada; só a ATRIBUIÇÃO de dia muda.
--   5) Branch @period: bounds ancorados com offset explícito SÓ para colunas
--      do núcleo (from → T00:00:00-03:00; to → -03:00/T23:59:59-03:00).
--      Literal com offset compara o instante exato independente do fuso da
--      sessão, direto contra a coluna (sargável — índices 0069 preservados).
--      Bounds de custom seguem VERBATIM: ancorar o lower bound excluiria
--      valores date-only ('2026-07-01' < '2026-07-01T00:00:00-03:00').
--      Brasil não tem DST desde 2019 — offset fixo -03:00, mesma convenção de
--      lib/date/normalize.ts e do backfill 0080.
--
-- Efeitos colaterais documentados: (a) transform 'none' em coluna de data do
-- núcleo/match segue serializando no fuso da sessão (agrupar por instante é
-- bijetivo; só o prefixo do rótulo cru fica em UTC — follow-up conhecido);
-- (b) match: para campo custom do parceiro com transform de data antes
-- lançava erro de tipo em runtime, agora resolve pelo overload text (null p/
-- valor inválido); (c) eq/eq_ci em coluna de data do núcleo seguem intocados
-- (comparação de instante exato — degenerada antes e depois); (d) snapshots
-- antigos com valores pré-0080 (offset de Moscou) bucketizam pelo prefixo
-- CAPTURADO — consistente com as listas do próprio snapshot.
--
-- Recria run_widget_query (base 0072) e ESPELHA run_widget_query_snapshot na
-- MESMA migração. Idempotente (create or replace). Aplicar ANTES do deploy do
-- app (a mudança é tolerante a ordem: literal ancorado do cliente novo também
-- compara certo no RPC antigo).

-- ---- Helper: instante -> wall time de Brasília (timestamp, sem tz) ----
create or replace function public._widget_local_ts(p timestamptz)
returns timestamp
language sql
immutable
set search_path = ''
as $$
  select p at time zone 'America/Sao_Paulo'
$$;

-- ---- Helper: texto ISO -> dia pelo PREFIXO (timestamp; inválido -> null) ----
-- Prefixo de 10 chars de propósito: casa com o parseYmd do cliente e preserva
-- o dia literal de valores naive (CSV) e date-only. Horário é descartado — os
-- transforms dos RPCs são todos >= dia (não existe bucket por hora).
create or replace function public._widget_local_ts(p text)
returns timestamp
language plpgsql
immutable
set search_path = ''
as $$
begin
  return left(p, 10)::timestamp;
exception when others then
  return null;
end;
$$;

-- ---- Helper: ref -> expressão de DATA no dia de Brasília (timestamp) ----
-- custom:<k> pelo prefixo (overload text); coluna do núcleo convertida ao
-- wall time (overload timestamptz). Retorno homogêneo: o coalesce de
-- _widget_unified_date_expr não precisa mudar.
...
begin
  ...
  if p_ref like 'custom:%' then
    return format('public._widget_local_ts(custom_fields ->> %L)', substring(p_ref from 8));
  elsif p_ref = any(v_date_cols) then
    return format('public._widget_local_ts(%I)', p_ref);
  else
...
-- ---- Recria _widget_unified_expr (0047): data do núcleo em dia de Brasília ----
-- coalesce exige tipos compatíveis; `::text` uniformiza. Coluna de DATA do
-- núcleo agora serializa via _widget_local_ts — '2026-06-30 22:00:00' (wall
-- time BRT) em vez de '2026-07-01 01:00:00+00' (sessão UTC) — para o parseYmd
-- do cliente ler o dia certo. Membros custom/text/bool byte-idênticos.
...
begin
  ...
  loop
    if not p_numeric and v_ref = any(v_date_cols) then
      v_parts := v_parts
        || format('(public._widget_local_ts(%I))::text', v_ref);
    else
