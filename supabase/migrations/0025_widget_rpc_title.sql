...
-- Expõe a coluna `records.title` (Nome do Lead/Negócio — TITLE do Bitrix) no
-- construtor de widgets. A coluna já era sincronizada, mas estava fora das
-- whitelists do RPC, então nenhuma dimensão/métrica/filtro podia referenciá-la.
-- Recria as mesmas funções de 0020_widget_rpc_sources.sql apenas acrescentando
-- 'title' às duas arrays v_allowed_cols (helper + run_widget_query). 'title' é
-- texto: não entra em v_num_cols/v_date_cols. Idempotente (create or replace).
