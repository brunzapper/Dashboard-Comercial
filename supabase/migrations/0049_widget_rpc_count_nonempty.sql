...
-- Contagem por campo ignora STRINGS VAZIAS: `Contagem de <campo>` (agg count
-- sobre custom/unified/match) passa de count(expr) para count(nullif(expr, '')).
-- count() já ignora null, mas um valor '' gravado por importação/dado legado
-- era contado como "preenchido" — inflando razões como
-- Contagem(Data da assinatura) ÷ Contagem(Data Reunião). Comportamento para
-- null não muda; sum/avg já usavam nullif(expr, '') (sem alteração).
-- Recria run_widget_query a partir de 0048_widget_rpc_bucket_filter.sql.
