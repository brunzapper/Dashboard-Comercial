...
-- Dimensões de data "por nome": adiciona os transforms month_name, month_year,
-- week_year e week_month ao run_widget_query. O SQL só produz o BUCKET (uma data
-- agrupável); o rótulo em PT-BR (Janeiro, Janeiro/26, 5ª semana, 1ª semana de
-- Janeiro) é montado no engine (lib/widgets/date-buckets.ts). Mapeamento:
--   month_name/month_year -> date_trunc('month', col)   (agrupa por mês)
--   week_year              -> date_trunc('week', col)    (semana ISO, segunda)
--   week_month (full)      -> date_trunc('week', col)    (semana cheia seg→dom)
--   week_month (restricted)-> greatest(date_trunc('week',col), date_trunc('month',col))
-- O modo da semana vem em cada dimensão como "weekMode". Recria a mesma função de
-- 0028_widget_rpc_ilike.sql, só ampliando o ramo de transform das dimensões.
-- Idempotente (create or replace). Helpers _widget_col_expr/_widget_unified_expr
-- já existem (0020/0025).
