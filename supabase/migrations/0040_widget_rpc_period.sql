...
-- Adiciona ao run_widget_query o FILTRO sintético `@period` (operador 'between'),
-- que aplica o filtro de período com uma COLUNA DE DATA POR FONTE: o valor traz
-- os limites + o mapa record_type → coluna de data e é expandido num OR por tipo
-- (ex.: negócios por `closed_at`/assinatura e Estudo/venda_site por
-- `source_created_at`, na MESMA seleção de calendário). Assim uma seleção única
-- filtra cada fonte pela sua própria data — corrige o Estudo de Fechamentos, cujo
-- `closed_at` é sempre NULL. Recria a função de 0039_widget_rpc_rate_date.sql
-- apenas acrescentando esse ramo no loop de filtros. Idempotente.
...
begin
  ...
  loop
    ...
    if v_field = '@rate_date' then
      -- Data sintética p/ escolher a taxa de câmbio: 1ª data disponível do registro
      -- (fechamento → abertura → criação), igual ao recYQ do modo registros.
  ...
  loop
    ...
    -- Período por fonte: campo sintético `@period` (op 'between'). O valor traz
    -- os limites + o mapa record_type → coluna de data; expande num OR por tipo,
    -- p/ cada fonte filtrar pela sua própria data na MESMA seleção de calendário.
