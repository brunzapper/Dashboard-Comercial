-- 0105_widget_rpc_in_ci.sql
-- Versão: 1.0 | Data: 26/07/2026
-- Parcerias (fase B): novo operador INTERNO `in_ci` no par de RPCs de widgets —
-- pertencimento com a MESMA normalização do eq_ci (0050, _widget_norm_text).
-- Usado exclusivamente pela FUSÃO de perfis de operação no engine
-- (lib/config/operation-scope.ts): multi-seleção de operações definidas por
-- perfil (ex.: sub-operações de parceria) vira UM filtro `in_ci` com a união
-- dos valores — antes os perfis eram descartados e o dashboard zerava. Fundir
-- em `in` cru seria perda silenciosa (o `in` compara ::text case-sensitive).
-- Fora da UI de filtros e do SPEC da IA (mesmo precedente do eq_ci).
-- Recria run_widget_query (corpo 0085 verbatim + ramo novo) e ESPELHA
-- run_widget_query_snapshot na MESMA migração (invariante 1; fiscalizado por
-- tests/rpc-parity.test.ts). Helpers intocados. Idempotente.
