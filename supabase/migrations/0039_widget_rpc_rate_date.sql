...
-- Adiciona ao run_widget_query um campo sintético de DIMENSÃO `@rate_date`, que
-- resolve para coalesce(closed_at, opened_at, source_created_at) e aceita os
-- transforms year/quarter. Serve à agregação monetária dos widgets agregados
-- (lib/widgets/engine.ts): a "data da taxa" de conversão de cada registro é a
-- mesma do modo registros individuais (recYQ), garantindo paridade — inclusive
-- para negócios em aberto (sem closed_at). Recria a função de
-- 0035_widget_rpc_weekday.sql apenas acrescentando esse ramo. Idempotente.
