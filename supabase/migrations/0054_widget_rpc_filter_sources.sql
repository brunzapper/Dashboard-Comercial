...
-- Filtros segmentados por fonte (pass-through): um filtro pode carregar
-- `record_types: ["lead", ...]` e então só restringe as linhas dessas fontes —
-- as demais fontes do widget passam sem restrição:
--   (record_type not in (<alvos>) or (<predicado>))
-- Filtro sem `record_types` = comportamento atual (vale p/ todas as fontes),
-- então p_filters antigos seguem idênticos. O wrapper é aplicado a TODO
-- predicado do loop de filtros (inclusive @period/@bucket, que hoje nunca
...
-- wrapper. Injeção-segura: os alvos entram como literal jsonb (%L) desfeito
-- por jsonb_array_elements_text — mesmo padrão do ramo `in` do filtro.
-- record_type é text NOT NULL (0004), então o `not in` nunca tropeça em null.
create or replace function public._widget_wrap_record_types(
  p_pred text,
...
as $$
begin
  if p_pred is null or p_pred = '' then
    return p_pred;
  end if;
  if p_record_types is null
     or jsonb_typeof(p_record_types) is distinct from 'array'
     or jsonb_array_length(p_record_types) = 0 then
    return p_pred;
  end if;
  return format(
    '(record_type not in (select jsonb_array_elements_text(%L::jsonb)) or (%s))',
    p_record_types::text, p_pred
  );
end;
$$;

grant execute on function public._widget_wrap_record_types(text, jsonb) to authenticated;
