...
// v1.2 (09/09/2026): a ação `create_task_series` é RECUSADA pelo mesmo motivo
//   do run_schema — ela cobra o responsável periodicamente, sozinha.
    ...
    if (!Array.isArray(obj.automacoes)) {
      ...
      obj.automacoes.forEach((item, i) => {
        ...
        if (rule.action.type === "create_task_series") {
          // Uma série cobra o vendedor a cada quinzena, sozinha, por tempo
          // indeterminado. Quem a liga assume isso — não é coisa que nasça de
          // JSON gerado.
          errors.push(
            `${where} ("${nome}"): a ação "série de tarefas" precisa ser criada à mão no painel de automações — ela cobra o responsável periodicamente, sem ninguém olhando.`
          );
          return;
        }
