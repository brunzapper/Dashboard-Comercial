...
function input(){
  ...
  return {revision:"2026-09-21T10:00:00Z",accessVersion:1,image:`data:image/webp;base64,${bytes.toString("base64")}`,width:560,height:560,format:"tabs-v2"};
}
it("aceita só envelope WebP pequeno com revisão/epoch/dimensões válidas",()=>{
  expect(validPreviewUpload(input())).not.toBeNull();
  for(const patch of [{format:undefined},{format:"old"},{accessVersion:null},{revision:"invalid"},{width:0},{image:"data:image/svg+xml,<svg/>"},{image:"data:image/webp;base64,"+"A".repeat(60000)}])expect(validPreviewUpload({...input(),...patch})).toBeNull();
